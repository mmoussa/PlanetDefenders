...
Shader "Custom/Toon-Shader" {
   Properties {
    ...
    float3 combinedLight = (diffuseReflection) * outlineStrength + specularReflection;
           
    return float4(combinedLight, 1.0) * (tex2D(_MainTex, input.uv)*outlineStrength); // DELETE LINE COMMENTS & ';' TO ENABLE TEXTURE
